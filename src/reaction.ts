...
export default class Reaction {
  ...
  static track(
    ...
    target: Map<string | symbol | number, Set<Reaction>>,
  ...
  static getReactionsForOperation({
    target,
    key,
    type,
  }: IOperation): Set<Reaction> {
    ...
    if (type === "add" || type === "delete" || type === "clear") {
      Reaction.track(
        reactionsForKey,
        reactionsForRaw,
        Array.isArray(target) ? "length" : Reaction.ITERATOR_KEY
      );
    }
  ...
  static runningReactions({ target, key, type }: IOperation) {
    Reaction.getReactionsForOperation({
      target,
      key,
      type,
    }).forEach((reaction) => reaction.run());
  }
