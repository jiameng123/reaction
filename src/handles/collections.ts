...
const collections = {
  ...
  has(key: any) {
    const { target, proto } = getTargetAndProto(this);
    const result = proto.has.call(target, key);

    Reaction.register({ target, key, type: "has" });
    return result;
  },
  ...
  forEach(cb: Function, ...args) {
    ...
    proto.forEach.call(target, wrapCb, ...args);
  },
  ...
  clear() {
    ...
    proto.clear.apply(target);
    if (hadItems) {
      Reaction.runningReactions({ target, type: "clear" });
    }
  },
};
